...
class StalaQuizApp:
    ...
    def fetch_user_score_and_played(self, username):
        try:
            ...
            st = "SELECT score, played FROM player WHERE usrname = %s"
            self.cursor.execute(st, (username_to_search,))
            data = self.cursor.fetchone()
            if data:
    ...
    def fetch_played_question_count(self, username):
        try:
            username_to_search = hashlib.md5(username.encode()).hexdigest()
            # Replace string formatting with parameterized query
            st = "SELECT played FROM player WHERE usrname = %s"
            self.cursor.execute(st, (username_to_search,))
            data = self.cursor.fetchone()
            if data:
                return data[0]
    ...
    def end_quiz(self):
        try:
            ...
            # Replace string formatting with parameterized query
            st = "UPDATE player SET score = %s, played = %s WHERE usrname = %s"
            self.cursor.execute(st, (self.score, self.played, username_to_search))
            self.mycon.commit()
